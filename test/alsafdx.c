 ...
 *  By Jaroslaw Kysela.
 *  Modified by Günther Montag for send and receive 
 *  in direct_write loop, as a preparation for 
 *  alsa fdx for hfkernel.
 */
...
// playback
		...
		while (psize > 0) {
			pframes = psize;
			err = snd_pcm_mmap_begin(handle, &my_playback_areas, &poffset, &pframes);
			if (err < 0) {
				if ((err = xrun_recovery(handle, err)) < 0) {
					printf("MMAP begin avail error: %s\n", snd_strerror(err));
					exit(EXIT_FAILURE);
			...
			commitres = snd_pcm_mmap_commit(handle, poffset, pframes);
...
// capture 
		...
		while (csize > 0) {
			cframes = csize;
			err = snd_pcm_mmap_begin(chandle, &my_capture_areas, &coffset, &cframes);
			if (err < 0) {
				if ((err = xrun_recovery(chandle, err)) < 0) {
					printf("MMAP begin avail error: %s\n", snd_strerror(err));
					exit(EXIT_FAILURE);
				...
				cfirst = 1;
                	...
                	commitres = snd_pcm_mmap_commit(chandle, coffset, cframes);
			if (commitres < 0 || commitres != cframes) {
				if ((err = xrun_recovery(chandle, commitres >= 0 ? -EPIPE : commitres)) < 0) {
					printf("MMAP commit error: %s\n", snd_strerror(err));
					exit(EXIT_FAILURE);
				}
				cfirst = 1;
...
int main(int argc, char *argv[])
{
	struct option long_option[] =
	{
        ...
        snd_pcm_t *handle;                             /* playback device */
        int err, morehelp;
	...
	if ((err = set_hwparams(chandle, hwparams, transfer_methods[method].access)) < 0) {
	...
	if ((err = set_hwparams(phandle, hwparams, transfer_methods[method].access)) < 0) {
	...
	err = transfer_methods[method].transfer_loop(phandle, samples, areas);
	if (err < 0)
