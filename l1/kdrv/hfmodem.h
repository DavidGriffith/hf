...
#if defined(__i386__) && defined(CONFIG_HFMODEM_FLOAT)

typedef struct {
	unsigned char buf[108];
} hfmodem_fpu_state_t;

typedef float hfmodem_conv_t;

extern __inline__ void hfmodem_fpu_save(hfmodem_fpu_state_t *s)
{
	__asm__("fnsave %0;\n\t" : "=m" (*s) : : "memory");
}

extern __inline__ void hfmodem_fpu_restore(hfmodem_fpu_state_t *s)
{
	__asm__("frstor %0;\n\t" : : "m" (*s));
}
