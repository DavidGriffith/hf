...
extern __inline__ hfmodem_soft_t do_filter(struct hfmodem_l1_rxslot *slot, short *s)
{
	...
	if (cc >= HFMODEM_CORRELATOR_CACHE) {
		printk(KERN_ERR "%s: do_filter: correlator cache index overrange\n", hfmodem_drvname);
		return 0;
	}
	return (fsqr(fcorr(slot->corrlen, hfmodem_correlator_cache[cc].correlator[1][0], s)) +
		fsqr(fcorr(slot->corrlen, hfmodem_correlator_cache[cc].correlator[1][1], s)) -
		fsqr(fcorr(slot->corrlen, hfmodem_correlator_cache[cc].correlator[0][0], s)) -
		fsqr(fcorr(slot->corrlen, hfmodem_correlator_cache[cc].correlator[0][1], s))) 
		* slot->scale;
}
...
{
	hfmodem_fpu_state_t fpu;
	unsigned int j, k, l, ph, phinc;

	hfmodem_fpu_save(&fpu);
	...
	if (j >= HFMODEM_CORRELATOR_CACHE) {
		...
		for (k = 0; k < 2; k++) {
			...
			for (ph = l = 0; l < HFMODEM_MAXCORRLEN; l++, ph = (ph + phinc) & 0xffff)
				ftblcossin(ph, &hfmodem_correlator_cache[j].correlator[k][0][l],
	...
	slot->corr_cache = j;
	cc_lock(j);
	hfmodem_fpu_restore(&fpu);
}

/* --------------------------------------------------------------------- */

#else /* CONFIG_HFMODEM_FLOAT */
...
//printk(KERN_DEBUG "%s: ioctl: %d  arg: 0x%lx\n", hfmodem_drvname, cmd, arg);
	...
	case HFMODEM_IOCTL_MIXERPARAMS:
		if ((ret = copy_from_user(&mix, (void *)arg, sizeof(mix))))
			return ret;
printk(KERN_DEBUG "%s: mix par: %d %d %d\n", hfmodem_drvname, mix.src, mix.igain, mix.ogain);
