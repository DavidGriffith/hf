...
static pthread_cond_t l1cond = PTHREAD_COND_INITIALIZER;
static pthread_mutex_t l1mutex = PTHREAD_MUTEX_INITIALIZER;
...
} l1rxslots[NUMRXSLOTS];

static struct l1txslots {
	enum l1_slot_st state;
	l1_time_t tstart, tinc;
	unsigned char *data;
	unsigned int nbits;
...
unsigned long l1_wait_request(void)
{
	int i, idx, cnt;
	unsigned long id;
	l1_time_t tm = 0;
	...
	for (;;) {
		...
		if (cnt <= 0)
			errprintf(SEV_FATAL, "l1_wait_request: no requests pending");
		      ...
		      short *samples, unsigned int nsamples)
{
	...
	for (i = 0; i < NUMRXSLOTS; i++) {
		if (l1rxslots[i].state == l1ss_unused || l1rxslots[i].state == l1ss_retired)
			continue;
...
long l1_next_tx_event(l1_time_t current)
{
...
void l1_finish_pending_rx_requests(void)
{
	int i;
