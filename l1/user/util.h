...
/*
 *      util.h  --  binary FSK modem code, utility function.
 *
...
extern __inline__ unsigned int cc_replace(void)
{
	...
	if (k < CORRELATOR_CACHE)
		return k;
	errprintf(SEV_WARNING, "modem: out of filter coefficient cache entries\n");
...
static void cc_prepare(struct l1rxslots *slot, unsigned short phinc0, unsigned short phinc1)
{
	...
	if (j >= CORRELATOR_CACHE) {
		...
		for (k = 0; k < 2; k++) {
			...
			for (ph = l = 0; l < L1CORR_LEN; l++, ph = (ph + phinc) & 0xffff)
				ftblcossin(ph, &corr_cache[j].correlator[k][0][l],
...
static void cc_prepare(struct l1rxslots *slot, unsigned short phinc0, unsigned short phinc1)
{
	...
	if (j >= CORRELATOR_CACHE) {
		...
		for (k = 0; k < 2; k++) {
			phinc = corr_cache[j].phase_incs[k];
			for (ph = l = 0; l < L1CORR_LEN; l++, ph = (ph + phinc) & 0xffff)
				itblcossin(ph, &corr_cache[j].correlator[k][0][l],
					   &corr_cache[j].correlator[k][1][l]);
		}
		corr_cache[j].refcnt = 0;

#if 0
		printk("corr: %u ph: 0x%04x 0x%04x\n", j, corr_cache[j].phase_incs[0], corr_cache[j].phase_incs[1]);
		for (l = 0; l < MAXCORRLEN; l++)
			printf("corr: %6d %6d %6d %6d\n", corr_cache[j].correlator[0][0][l],
			       corr_cache[j].correlator[0][1][l], corr_cache[j].correlator[1][0][l],
