...
static void start_l2_thread(void *(*routine)(void *))
{
#ifndef NOREALTIME
...
static void process_msg(struct hfapp_msg *msg)
{
	...
	case HFAPP_MSG_SET_PACTORPAR:
		if (len < sizeof(msg->data.ppar))
			return;
		pactor_set_params(msg->data.ppar.destcall, msg->data.ppar.mycall,
				  ntohs(msg->data.ppar.txdelay), ntohs(msg->data.ppar.retry),
				  msg->data.ppar.longpath, ntohs(msg->data.ppar.crcpreset[0]), 
				  ntohs(msg->data.ppar.crcpreset[1]), ntohs(msg->data.ppar.crcpreset[2]), 
	...
	case HFAPP_MSG_SET_GTORPAR:
		if (len < sizeof(msg->data.gpar))
			return;
		gtor_set_params(msg->data.gpar.destcall, msg->data.gpar.mycall,
				  ntohs(msg->data.gpar.txdelay), ntohs(msg->data.gpar.retry));
		return;
	...
	case HFAPP_MSG_SET_AMTORPAR:
		if (len < sizeof(msg->data.apar))
			return;
		amtor_set_params(msg->data.apar.destcall, msg->data.apar.selfeccall, msg->data.apar.mycall,
				 htons(msg->data.apar.txdelay), htons(msg->data.apar.retry), 
				 msg->data.apar.rxinvert, msg->data.apar.txinvert);
		return;
	...
	case HFAPP_MSG_SET_RTTYPAR:
		if (len < sizeof(msg->data.rpar))
			return;
		rtty_set_params(htons(msg->data.rpar.baud), msg->data.rpar.rxinvert, msg->data.rpar.txinvert);
		return;
	...
	case HFAPP_MSG_SET_FSKPAR:
		if (len < sizeof(msg->data.fpar))
			return;
		l1_set_fsk_params(htons(msg->data.fpar.freq[0]), htons(msg->data.fpar.freq[1]));
		return;
		
	case HFAPP_MSG_SET_MIXERPAR:
		if (len < sizeof(msg->data.mpar))
			return;
		l1_set_mixer(htonl(msg->data.mpar.src), htonl(msg->data.mpar.igain), htonl(msg->data.mpar.ogain));
		return;
...
void *io_process(void *dummy)
{
	...
	for (;;) {
		...
		/* send keepalive message on socket */
		if (fd_app >= 0) {
